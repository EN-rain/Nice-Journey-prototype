...
- Latest combat-integrated local broad gate: `docs/evidence/combat-floor1-live-playtest-validation-local.json` reports **279 dynamically discovered tests, 0 failures** on 2026-09-20, including the new three-class Floor 1 scripted physical fight and first equipped rank-one active skill, opt-in combat telemetry, the 36-scenario normal-damage controlled class/archetype baseline, enemy projectile impact/replay, status/defender validation, equipment authoring and three classes' route/death-retry checks. Headless editor parse, main-scene smoke and GL Compatibility renderer-active reference smoke passed. `docs/evidence/combat_floor1_live_playtest_local.json` measures one live Floor 1 Duelist per class with a clear stand-off, scripted 32-px combat-position reset, moving AI, physical contact/projectiles, real basic+rank-one skill and incoming damage. It is NOT human-input, full-encounter survival, twelve-archetype or boss fairness evidence. The earlier `combat_balance_baseline_local.json` is stationary nonretaliating Floor-5 data. Numeric combat/boss/equipment/status content remains PLAYTEST; reference-machine frame/memory measurements remain greybox only, not final DR-08/minimum-hardware acceptance. This working-tree gate includes concurrent uncommitted art/UI changes. The recurring `Save rejected: invalid profile_id` line is the intentional negative fixture.
...
`test_three_class_live_death_retry.gd` additionally confirms Melee/Ranged/Mage real Floor-1 combat defeat, enemy-caused player death, modal retry, exact durable floor/defeat rollback, reactivated combat/objective and one durable Quest Hall XP reward. Test damage magnitudes and trigger emission are controlled; human-input survival, Escort/defense under simultaneous enemy pressure and final combat balance remain open.
