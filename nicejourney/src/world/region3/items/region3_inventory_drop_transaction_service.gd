class_name Region3InventoryDropTransactionService
extends RefCounted
...
const REASON_REGION_STATE_UNAVAILABLE: StringName = &"region_state_unavailable"
...
static func drop_full_stack(
    ...
    if not _base_inputs_valid(profile, layout, transaction_id):
        return _result(false, REASON_INVALID_INPUT)
    if not StableId.is_valid(String(item_instance_id)) or not _finite_position(local_position):
        return _result(false, REASON_INVALID_INPUT)
    var position_context := Region3SubzoneDiscoveryService.zone_id_at_local_position(layout, local_position)
    if not bool(position_context.get("accepted", false)):
        return _result(false, REASON_INVALID_INPUT)

    var states := _load_states(profile, layout)
    ...
    var region_state := (states["region_state"] as Dictionary).duplicate(true)
    ...
    if inventory.protected_items.has(String(item_instance_id)):
        var policy := ProtectedItemOperationPolicy.evaluate(
    ...
    var item := inventory.get_normal_slot(item_instance_id)
    if item.is_empty():
        return _result(false, REASON_ITEM_NOT_FOUND)

    var source_id := StringName("%s%s" % [DROP_SOURCE_PREFIX, String(transaction_id)])
    var loose_items := (region_state.get("loose_items", {}) as Dictionary).duplicate(true)
    if loose_items.has(String(source_id)):
    ...
    var removed := temp_inventory.try_remove_normal(item_instance_id, owned_quantity)
    ...
    if not (PLAYER_DROP_STATE_VALIDATOR.validate_entry(entry, source_id) as PackedStringArray).is_empty():
        return _result(false, REASON_COMMIT_FAILED)
    loose_items[String(source_id)] = entry
    region_state["loose_items"] = loose_items
    if not temp_ledger.try_claim(transaction_id, source_id):
        return _result(false, REASON_DUPLICATE_TRANSACTION)

    var commit := _commit_profile(profile, temp_inventory, temp_ledger, region_state)
...
static func collect_player_drop(
    ...
    if not _base_inputs_valid(profile, layout, transaction_id) or not StableId.is_valid(String(source_id)):
        return _result(false, REASON_INVALID_INPUT)
    ...
    loose_items.erase(String(source_id))
    region_state["loose_items"] = loose_items
    if not temp_ledger.try_claim(transaction_id, source_id):
        return _result(false, REASON_DUPLICATE_TRANSACTION)
...
static func _load_states(profile: ProfileSnapshot, layout: Region3AuthoredTownLayout) -> Dictionary:
    if profile.item_state.is_empty():
        return {"accepted": false, "reason_id": REASON_ITEM_NOT_FOUND}
    if profile.region_state.is_empty():
        return {"accepted": false, "reason_id": REASON_REGION_STATE_UNAVAILABLE}
    var region_errors := Region3SubzoneDiscoveryService.validate_state_dictionary(profile.region_state)
    if not region_errors.is_empty():
        return {"accepted": false, "reason_id": REASON_REGION_STATE_UNAVAILABLE}
    if int(profile.region_state.get("region_id", 0)) != Region3SubzoneDiscoveryService.REGION_ID:
        return {"accepted": false, "reason_id": REASON_REGION_STATE_UNAVAILABLE}
    if StringName(String(profile.region_state.get("map_revision_id", &""))) != layout.map_revision_id:
        return {"accepted": false, "reason_id": Region3SubzoneDiscoveryService.REASON_MAP_REVISION_MISMATCH}
    if not Region3SubzoneDiscoveryService.validate_loose_positions_for_layout(profile.region_state, layout).is_empty():
        return {"accepted": false, "reason_id": REASON_REGION_STATE_UNAVAILABLE}
    ...
    return {
        ...
        "inventory": inventory,
        "ledger": ledger,
        "region_state": profile.region_state.duplicate(true),
    }
...
static func _commit_profile(
    ...
    if not InventoryState.validate_dictionary(item_data).is_empty():
        return _result(false, REASON_COMMIT_FAILED)
    if not ClaimLedger.validate_dictionary(claim_data).is_empty():
        return _result(false, REASON_COMMIT_FAILED)
    if not Region3SubzoneDiscoveryService.validate_state_dictionary(region_state).is_empty():
        return _result(false, REASON_COMMIT_FAILED)
    ...
    candidate["region_state"] = region_state.duplicate(true)
    ...
    profile.region_state = region_state
