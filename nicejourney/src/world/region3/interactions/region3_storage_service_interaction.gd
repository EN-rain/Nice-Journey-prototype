class_name Region3StorageServiceInteraction
extends Area2D
...
func request_service(commit_id: StringName) -> Dictionary:
    ...
    var target_valid := _authored_binding_valid and anchor != null and anchor.role_id == STORAGE_ROLE_ID
...
func validate_authored_binding() -> PackedStringArray:
    ...
    var anchor := get_parent() as Region3TownStructureAnchor
    if anchor == null:
        errors.append("storage service interaction must be a child of a Region3TownStructureAnchor")
        return errors
    if anchor.category != Region3TownStructureManifestValidator.CATEGORY_FUNCTIONAL:
        errors.append("storage service interaction requires a functional structure anchor")
    if anchor.role_id != STORAGE_ROLE_ID:
        errors.append("storage service interaction must belong to the storage_house role")
    var layout := anchor.get_parent() as Region3AuthoredTownLayout
    if layout == null:
        errors.append("storage service interaction requires the authored Region 3 town layout")
        return errors
    ...
    if collision_shape == null or not collision_shape.shape is CircleShape2D:
        errors.append("storage service interaction requires a CircleShape2D")
    return errors
...
func _unhandled_input(event: InputEvent) -> void:
    ...
    if not _input_ownership.can_route_gameplay_action(&"interact"):
        return
    _commit_sequence += 1
    var commit_id := StringName("interaction:region3_storage:%d" % _commit_sequence)
